...
#Get HTML file of page from URL
def getHTML(URL):
    hdr = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/47.0.2526.106 Safari/537.36"} #Lets YouTube know we are not a bot
    response = urllib.request.Request(URL, headers = hdr)                                                                                  #Request page
    response = urllib.request.urlopen(response)                                                                                            #Store the response
    response = response.read()                                                                                                             #Read the page's HTML 
...
def getStreamList(html):
    ...
    for video_stream in parseHTML(html)['url']:
        if "googlevideo" == video_stream.split('/')[2].split('.')[1]:   #Filters out bad videos eg: missing audio/video
...
#Gets all YouTube video URLs found on a page
